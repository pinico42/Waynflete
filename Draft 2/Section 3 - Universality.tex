...
Each element of the list will be a pair: firstly, the symbol which should be checked for, as a real number; secondly, the transformation that should be performed on the tape. This transformation will be of the form $(\bm{C})(\{\bm{M}_L/\bm{M}_R/\bm{M}_S\})(\bm{W}a)$, which applies first a write transformation first, before a movement transformation and the tape length correction transformation. 
