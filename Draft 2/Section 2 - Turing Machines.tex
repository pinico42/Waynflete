...
Since we have now entirely defined the operation of the machine in terms of functions (as loops can be done with recursion), we no longer have any need for tables - in effect, we have built a purely functional programming language in terms of a simple finite state machine. Now, in order to define more \textit{m}-functions we no longer have to write a table:
...
Admittedly not the cleanest syntax, but still a simple one.
\end{document}
