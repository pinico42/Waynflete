...
\medskip\noindent\begin{tabu} to \textwidth{XXXX}
    ...
    \textit{m-config} & \textit{symbol} & \textit{operations} & \textit{final m-config} \\
    \hhline{====}
    \multirow{1}{*}{$\mathbb{Q}$} & Any        & P\texttt{e}, R P\texttt{e}, R,  P\texttt{e}, R, R, P\texttt{0}, L, L & $\mathbb{A}$ \\
    \hhline{----}
    \multirow{2}{*}{$\mathbb{A}$} & \texttt{1} & R, P\texttt{x}, L, L, L                   & $\mathbb{A}$ \\
                                  & \texttt{1} &                                  & $\mathbb{B}$ \\
    \hhline{----}
...
\medskip\noindent\begin{tabu} to \textwidth{XXXXX}
    ...
    \hhline{====}
    $\mathbb{fl}$(\textit{a}, \textfrak{C}, \textfrak{e}) & - & - & $\mathbb{f}$($\mathbb{l}$(\textit{a}, \textfrak{C}), \textfrak{E}) & Moves to left of \textit{a} \\
    \hhline{----}
    $\mathbb{fr}$(\textit{a}, \textfrak{C}, \textfrak{e}) & - & - & $\mathbb{f}$($\mathbb{r}$(\textit{a}, \textfrak{C}), \textfrak{E}) & Moves to right of \textit{a} \\
    \hhline{====}
    $\mathbb{p0}$(\textfrak{C}) & - & P\texttt{0} & \textfrak{C} & Prints \texttt{0} \\
    \hhline{----}
    $\mathbb{p1}$(\textfrak{C}) & - & P\texttt{1} & \textfrak{C} & Prints \texttt{0} \\
    \hhline{====}
    $\mathbb{n}$(\textfrak{C}, \textfrak{E}) & - & - & $\mathbb{f}$($\mathbb{n}_1$(\texttt{e}, \textfrak{C}), \textfrak{E}) & \multirow{3}{80pt}{Moves to first empty (final) \textit{p}-square} \\
    \hhline{----}
...
\medskip\noindent\begin{tabu} to \textwidth{XXXX}
    ...
    \hhline{----}
    \multirow{3}{*}{$\mathbb{\mathbf{b}neg}_1$(\textit{w},\textfrak{C},\textfrak{E})} & \texttt{0} & - & $\mathbb{fl}$($\mathbb{p1}$(\textfrak{C}),\textfrak{E},\textit{w}) \\
                                                                                       & \texttt{1} & - & $\mathbb{fl}$($\mathbb{p0}$(\textfrak{C}),\textfrak{E},\textit{w}) \\ 
                                                                                       & other & - & \textfrak{E} \\
\end{tabu}
...
Since we have now entirely defined the operation of the machine in terms of functions (as loops can be done with recursion), we no longer have any need for the central two table columns. In continuing to define functions, we could simply choose to omit these columns, or even omit the table altogether and use a more common notation - in this case, for a definition of the if function but based on pointers: 
...
\end{lstlisting}
While this syntax is more concise, it is still somewhat obfuscated. Therefore, we will use our function method to define what is effectively a programming language.
\end{document}
