...
With the Z3's two new-found tools, production of a universal machine is incredibly simple. The machine would first be prepared with the initial state (an integer) at the address $s$, and the initial index of the cursor (also an integer, $a$ by default) at the address $h$. Then the following program would be run repeatedly with the ends of the tape attached to each other.
\begin{gather*}
\begin{aligned}
&r = (h)						\\
&p = k 							\\
